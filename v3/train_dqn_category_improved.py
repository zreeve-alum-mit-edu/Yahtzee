...
from dqn_category_improved import DQNCategoryPlayer
import time
import argparse
...
def play_vectorized_episodes(games, player, training=True):
    ...
    for round_num in range(games.total_rounds):
        ...
        if training:
            # Create done tensor
            dones = torch.full((num_games,), done, dtype=torch.float32, device=games.device)

            # Store all transitions from this batch
...
def train(Z=3, num_episodes=10000, num_parallel_games=256, batch_size=64,
          ...
          updates_per_step=1):
    """
    Train DQN agent for category selection in Multi-Yahtzee with vectorized games.
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % save_freq == 0:
            checkpoint_path = f"dqn_category_improved_z{Z}_ep{episode+1}.pth"
            player.save(checkpoint_path)
            print(f"Saved checkpoint: {checkpoint_path}")

    # Final save
    final_path = f"dqn_category_improved_z{Z}_final.pth"
...
def plot_training_curves(episode_rewards, eval_rewards, losses, epsilons, save_freq):
    ...
    plt.tight_layout()
    plt.savefig('dqn_training_curves_improved.png', dpi=150)
    plt.show()


def main():
    parser = argparse.ArgumentParser(description='Train DQN for Multi-Yahtzee Category Selection (Improved)')
