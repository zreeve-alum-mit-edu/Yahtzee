...
class TestDiceSorting(unittest.TestCase):
    ...
    def test_hold_mask_applies_to_sorted_positions(self):
        ...
        hold_mask = torch.tensor([[True, True, False, False, False]], dtype=torch.bool, device=game.device)
    ...
    def test_multiple_holds_maintain_sorting(self):
        ...
        for _ in range(5):
            ...
            # Random hold pattern
            hold_mask = torch.randint(0, 2, (2, 5), dtype=torch.bool, device=game.device)
            game.roll_dice(hold_mask)
    ...
    def test_identical_dice_sorting(self):
        ...
        hold_mask = torch.tensor([[True, False, True, False, True]], dtype=torch.bool, device=game.device)
        game.roll_dice(hold_mask)
