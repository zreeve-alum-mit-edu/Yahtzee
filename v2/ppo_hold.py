...
from constants import DICE_HOLD_MASKS
...
class PPOHoldPlayer:
    ...
    def train_flattened(self, trajectory):
        ...
        with torch.no_grad():
            S_all = torch.cat(states, dim=0)  # (T*B, state_dim)
            V_all = self.value_net(S_all).squeeze(-1)  # (T*B,)
            V = V_all.view(T, B)  # Reshape back to (T, B)
        ...
        if hold_indices:
            ...
            # Vectorized conversion from hold masks to action indices
            A_hold = []
            for i in hold_indices:
                hold_mask = actions[i][1]  # (B, 5)
                
                # Use broadcasting to compare all masks at once
                # hold_mask: (B, 5) -> (B, 1, 5)
                # self.hold_masks: (32, 5) -> (1, 32, 5)
                mask_comparison = (hold_mask.unsqueeze(1) == self.hold_masks.unsqueeze(0))  # (B, 32, 5)
                
                # Check which of the 32 masks match completely (all 5 positions)
                matches = mask_comparison.all(dim=2)  # (B, 32)
                
                # Get the index of the matching mask for each game
                # Using argmax since we know exactly one mask should match
                action_idx = matches.float().argmax(dim=1)  # (B,)
                
                A_hold.append(action_idx)
            A_hold = torch.cat(A_hold, dim=0)  # (26*B,)
        ...
        max_updates = self.k_epochs * ((len(hold_indices) * B // mb_size + 1) + 
                                       (len(cat_indices) * B // mb_size + 1))
