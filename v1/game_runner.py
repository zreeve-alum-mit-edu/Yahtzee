...
class GameRunner:
    ...
    def create_game(self, num_games):
        ...
        """
        self.game = SimpleYahtzee(num_games)
    ...
    def get_state(self):
        """
        Get current game state as a concatenated tensor.
        
        Returns:
            Tensor containing minimal game state
        """
        # Flatten dice from (x, 5, 6) to (x, 30)
        dice_flat = self.game.dice.view(self.game.num_games, -1)
        
        # Extract only whether categories are open (x, 6) - using the 7th column which is 1 if unscored
        categories_open = self.game.scores[:, :, 6]  # Shape: (num_games, 6)
        
        # Turn is already (x, 3)
        
        # Concatenate all state components: dice (30) + categories_open (6) + turn (3) = 39
        state = torch.cat([dice_flat, categories_open, self.game.turn], dim=1)
        return state
    
    def play_game(self):
        """
        Play a complete game of upper Yahtzee (6 rounds).
        ...
        # Play 6 rounds (one for each upper category)
